...
This project recreates the traditional Snake experience with a custom purple, pink, cyan, and black color theme. It focuses on real-time movement, keyboard controls, collision detection, random food generation, scoring, and game-state management.

## Project Overview

The goal of the game is to collect as much food as possible while keeping the snake alive.
...
The game ends when the snake touches the edge of the window or collides with its own body. After losing, the player can immediately start a new game or quit.
...
- Pink for the food and game-over message
- Cyan for the score
...
After food is collected, the allowed body length increases by one. The oldest segment remains in the list, causing the snake to grow.

### 4. Food Generation

Food appears at a random location that is aligned with the game's 10-pixel grid.

Before returning a new position, the game checks whether that position is already occupied by the snake. If it is, another position is generated. This prevents food from appearing underneath the snake's body.

### 5. Score System

The score begins at zero and increases by one whenever the snake collects food.

The current score is displayed in cyan in the upper-left corner of the game window.
...
Either collision ends the current run and opens the game-over screen.
...
Pressing `C` starts a completely new game with a one-block snake and a score of zero. Pressing `Q` closes the game.
...
The game window will open immediately. Press any arrow key to begin moving.
...
Generating food taught me how random values can be combined with validation. A random position is not automatically a valid position, so the game checks that the new food is not placed on the snake before using it.
...
- Show the final score more prominently
- Add keyboard instructions inside the game
